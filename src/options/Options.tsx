...
const settingsKeys: OptionsHashKey[] = Object.keys(optionsHashDefaults) as OptionsHashKey[]
...
function setOptionsHash(optionsHash: OptionsHash) {
  console.log(optionsHash)
  chrome.storage.sync.set(optionsHash).then(() => console.log("Value is set"));
}

export function Options() {
  const [curSettings, setCurSettings] = useState<OptionsHash>(optionsHashDefaults);
  const [syncs, setSyncs] = useState<number>(0);
  const [status, setStatus] = useState<JSX.Element>();

  const getSetCurSettings = () => getOptionsHash()
  .then(curSettings=>settingsKeys.reduce((curSettings: OptionsHash, key: keyof OptionsHash): OptionsHash => {
  ...
  return <div>
    ...
    <div>
      ...
      <select name="language" id="language" value={curSettings.gpt_language} onChange={(e:Event)=>{
          const ele = e.target as HTMLOptionElement
          setCurSettings({ ...curSettings, gpt_language: ele.value })
        }}>
        {GPT_LANGUAGE.map(lang=><option value={lang}>{lang}</option>)}
      </select>
    </div>
    ...
    <div style={{ margin: '10px' }}>
      <button onClick={e=>{
        setOptionsHash({
          // validations here
          gpt_language: GPT_LANGUAGE.includes(curSettings.gpt_language) ? curSettings.gpt_language : GPT_LANGUAGE_DEFAULT,
        })
      ...
      <button onClick={(e=>{
        setOptionsHash(optionsHashDefaults)
