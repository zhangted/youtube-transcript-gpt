...
async function getLangOptionsWithLink(videoId: string):Promise<transcriptLangOption[]> {
  const videoPageHtml = await fetch('https://www.youtube.com/watch?v=' + videoId)
    .then(res => res.text())
...
export async function getTranscript(videoId: string): Promise<transcriptPart[]> {
  const langOptLinks: transcriptLangOption[] = await getLangOptionsWithLink(videoId)
